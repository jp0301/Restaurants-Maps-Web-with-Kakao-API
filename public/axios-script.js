/*
┌───────────────────────────────────────┐
  1) 지도 생성 및 확대 축소 줌 컨트롤러
└───────────────────────────────────────┘
*/
var container = document.getElementById("map"); // 지도 영역 틀 DOM
var options = { // 지도 생성 시 옵션 설정
	center: new kakao.maps.LatLng(36.181962789725695, 127.11024263810567), //중심좌표
  level: 3, // 지도 얼만큼 확대할 것인지
};
...
async function getDataSet(category) {
  ...
  const dataSet = await axios({
    method: "get",
    url: `http://localhost:3001/restaurants?category=${qs}`,
    headers: {},
    data: {},
  });
